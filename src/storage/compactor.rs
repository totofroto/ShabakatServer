...
impl MetricsCompactor {
    ...
    async fn run_compaction_pass(db: &AppDb) -> Result<(), String> {
        ...
        let buckets_processed = db.execute(move |conn| -> Result<usize, String> {
            ...
            let rows = stmt.query_map(rusqlite::params![cutoff_time_ms], |row| {

                Ok((
                    row.get::<_, i64>(0).unwrap_or(0),     // device_id
                    row.get::<_, f64>(1).unwrap_or(0.0),   // avg_latency_ms
                    row.get::<_, f64>(2).unwrap_or(0.0),   // min_latency_ms
                    row.get::<_, f64>(3).unwrap_or(0.0),   // max_latency_ms
                    row.get::<_, i64>(4).unwrap_or(0),     // total_scans
                    row.get::<_, i64>(5).unwrap_or(0),     // hour_bucket
                ))
            ...
            for entry in &consolidated_entries {
                tx.execute(
                    ...
                    rusqlite::params![entry.0, entry.1, entry.2, entry.3, entry.4, entry.5],
                ).map_err(|e| e.to_string())?;
            }
