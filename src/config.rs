...
impl Config {
    pub fn from_env() -> Self {
        Self {
            ...
            // When auth is disabled via SHABAKAT_DISABLE_AUTH, a real secret is not
            // required — fall back to a dev-only placeholder so the server can start.
            jwt_secret: {
                let disable_auth = env::var("SHABAKAT_DISABLE_AUTH").map(|v| v == "true").unwrap_or(false);
                if disable_auth {
                    env::var("JWT_SECRET").unwrap_or_else(|_| "dev-bypass-secret-not-for-production".to_string())
                } else {
                    env::var("JWT_SECRET").expect("CRITICAL ERROR: JWT_SECRET environment variable is missing! The server cannot start safely.")
                }
            },
